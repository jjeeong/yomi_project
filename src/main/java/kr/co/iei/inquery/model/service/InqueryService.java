...
public class InqueryService {
	...
	//로그인 시 조회
	public InqueryListData selectInqueryList(int reqPage, Member member) { //reqPage : 사용자가 요청한 페이지 번호
		//numPerPage : 한 페이지당 보여줄 게시물의 수
		int numPerPage = 10;
		//사용자가 요청한 페이지에 따른 게시물 시작번호(start)/끝번호(end) 계산
		// reqPage == 1 -> start = 1 / end = 10
		...
		int start = end - numPerPage + 1;
		// start : 1(10-10+1), 11(20-10+1), 21(30-10+1), ... / end : : 10(1*10), 20(2*10), 30(3*10), ...
		List list = inqueryDao.selectInqueryList(start, end, member);		
		//페이지 네비게이션
		int totalCount = inqueryDao.selectInqueryTotalCount(member);
		//totalPage : 전체 페이지 수
		int totalPage = 0;
		if(totalCount % numPerPage == 0) {
			totalPage = totalCount / numPerPage;			
